...
function background() {
    ...
    [ "$(magisk -v 2>/dev/null | grep '14..*:MAGISK')" ] && {
        MAGISK_VERSION=14
        HIDELIST_FILE=/magisk/.core/hidelist; }

    [ "$(magisk -v 2>/dev/null | grep '17..*:MAGISK')" ] && {
        MAGISK_VERSION=17
        HIDELIST_FILE=/sbin/.core/img/.core/hidelist; }

    mkdir -p /data/magisk
    if [ $MAGISK_VERSION == 17 ]; then
      CPCMD="cp -af $MODDIR/busybox /data/adb/magisk/busybox"
    else
      CPCMD="cp -af $MODDIR/busybox /data/magisk/busybox"
    fi
    $CPCMD || {
...
### Universal Hide ###
    [ "$MAGISK_VERSION" == "12" ] || [ "$MAGISKHIDE" == "0" ] && {
        ...
        [ "$MAGISK_VERSION" == "14" ] && $RESETPROP --delete "persist.magisk.busybox"
...
BBX=/data/magisk/busybox
[ -e /data/adb/magisk/busybox ] && BBX=/data/adb/magisk/busybox
